...
test('Batch 1 scope: all other entries/pages, schema and Mapper UI remain byte-identical',()=>{
 ...
 const b2=['universe','human','abbey','abash','horizon','horse','media','aback','sound','abridge','aliment','acumen','abound','sky','language','advance','generate','absolute'];
 ...
 for(const dir of ['data/entries','words'])for(const file of fs.readdirSync(dir)){
  if([...slugs,'universe','man','abbey','abash','horizon','horse','media','aback','sound','abridge','aliment','acumen','abound','sky','language','advance','generate','absolute'].some(s=>file===s+'.html'||file===s+'.v1.json'))continue;

...
const current=beforeSecondPipeline(require('../data/language-book.v1.0.json')),data={...current,entries:current.entries.map(e=>['tier-c-batch3-0.1','tier-c-final-a-0.1'].includes(e.legacy_migration?.version)?legacyEntry(e):e)},before=JSON.parse(get('data/language-book.v1.0.json'));
...
test('Tier C Batch 2 only: other 38 records/pages, schema and UI remain unchanged',()=>{
 ...
 for(const dir of ['data/entries','words'])for(const f of fs.readdirSync(dir)){
  ...
  if([...slugs,'sky','language','advance','generate','absolute','a-indefinite-article'].some(s=>f===s+'.html'||f===s+'.v1.json'))continue;

...
const current=require('../data/language-book.v1.0.json'),data={...current,entries:current.entries.map(e=>['tier-c-batch3-0.1','tier-c-final-a-0.1'].includes(e.legacy_migration?.version)?legacyEntry(e):e)},before=JSON.parse(get('data/language-book.v1.0.json'));

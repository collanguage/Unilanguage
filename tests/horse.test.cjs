...
test('HORSE remains additive while the focused ABHOR record may advance independently',()=>{
 ...
 assert.deepEqual(data.entries.filter(x=>!['horse','abhor','abdicate','abbreviate','abbreviation','abdominal','abdomen'].includes(x.slug)),before.entries.filter(x=>!['abhor','abdicate','abbreviate','abbreviation','abdominal','abdomen'].includes(x.slug)));

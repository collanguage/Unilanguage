...
test('ABBEY recalibrates one identity without changing publication counts',()=>{
 ...
 assert.equal(data.entries.filter(e=>!['new','horizon','horse','abbreviation','abdominal'].includes(e.slug)).length,manifest.entry_count_before);
 assert.equal(data.entries.filter(e=>e.entry_status==='Published'&&!['horizon','abhor','horse','abdicate','abbreviate','abbreviation','abdominal'].includes(e.slug)).length,manifest.published_entry_count_before);

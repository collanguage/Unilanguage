...
const current=require('../data/language-book.v1.0.json'),data={...current,entries:current.entries.map(e=>e.legacy_migration?.version==='tier-c-batch2-0.1'?legacyEntry(e):e)},before=JSON.parse(get('data/language-book.v1.0.json'));
...
test('Pending batch changes only three entries; other 39 entries and pages/schema remain exact',()=>{
 ...
 for(const dir of ['data/entries','words'])for(const f of fs.readdirSync(dir)){
  if([...slugs,'abridge','aliment','acumen','abound'].some(s=>f===s+'.html'||f===s+'.v1.json'))continue;

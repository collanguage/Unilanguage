...
test('ABDICATE recalibrates one existing ID and preserves every other record and raw note', () => {
  ...
  assert.deepEqual(data.entries.filter(x => !['aberrant','abdicate','abbreviate','abbreviation','abdominal','abdomen'].includes(x.slug)), before.entries.filter(x => !['aberrant','abdicate','abbreviate','abbreviation','abdominal','abdomen'].includes(x.slug)));

...
test('Batch 2 scope and exact archive restoration: no other entry or page changes',()=>{
 ...
 assert.deepEqual(data.entries.filter(e=>!slugs.includes(e.slug)).map(e=>['horizon','horse','media','aback','sound'].includes(e.slug)?legacyEntry(e):e),before.entries.filter(e=>!slugs.includes(e.slug)));
 for(const dir of ['data/entries','words'])for(const file of fs.readdirSync(dir)){
  if([...pages,'horizon','horse','media','aback','sound'].some(s=>file===s+'.html'||file===s+'.v1.json'))continue;

...
test('Batch 2 scope and exact archive restoration: no other entry or page changes',()=>{
 ...
 assert.deepEqual(data.entries.filter(e=>!slugs.includes(e.slug)).map(e=>['horizon','horse','media','aback','sound','abridge','aliment','acumen','abound','sky','language','advance','generate','absolute','a-indefinite-article'].includes(e.slug)?legacyEntry(e):e),before.entries.filter(e=>!slugs.includes(e.slug)));
 for(const dir of ['data/entries','words'])for(const file of fs.readdirSync(dir)){
  if(isSecondPipelineFile(file))continue;
  if([...pages,'horizon','horse','media','aback','sound','abridge','aliment','acumen','abound','sky','language','advance','generate','absolute','a-indefinite-article'].some(s=>file===s+'.html'||file===s+'.v1.json'))continue;

...
test('Batch 2 scope and exact archive restoration: no other entry or page changes',()=>{
 ...
 assert.deepEqual(data.entries.filter(e=>!slugs.includes(e.slug)).map(e=>['horizon','horse'].includes(e.slug)?legacyEntry(e):e),before.entries.filter(e=>!slugs.includes(e.slug)));
 for(const dir of ['data/entries','words'])for(const file of fs.readdirSync(dir)){
  if([...pages,'horizon','horse'].some(s=>file===s+'.html'||file===s+'.v1.json'))continue;
  const p=dir+'/'+file;if(fs.statSync(p).isFile())assert.equal(fs.readFileSync(p,'utf8').replace(/\r\n/g,'\n'),get(p),p);
 }
 for(const p of ['data/language-book-entry.schema.v1.json','js/diachronic-mapping.js','js/semantic-mapper.js','semantic-mapper.html','dictionary.html','search.html'])assert.equal(fs.readFileSync(p,'utf8').replace(/\r\n/g,'\n'),get(p),p);

...
test("product pages advance to G.3 while preserving Package E/G governance", () => {
  ...
  assert.match(mapper, /Package G\.3/);
  assert.match(mapper, /language-book\.v0\.5\.json/);
  assert.match(loader, /language-book\.v0\.5\.json/);

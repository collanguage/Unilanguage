...
Current Dataset Expansion release: `1.0.2` (13 records; Schema remains `1.0.0`). Patch releases may add reviewed records and optional schema-compatible fields without changing the v1.0 canonical filename. The Namcha Barwa record demonstrates the optional `record_kind: named_entity` and `entity_types: [proper_name, place, named_entity]` extension; ordinary lexical records remain valid without those fields.

...
Current Dataset Expansion release: `1.0.5` (13 records; Schema remains `1.0.0`). Patch releases may add reviewed records and optional schema-compatible fields without changing the v1.0 canonical filename. The Namcha Barwa record demonstrates the optional `record_kind: named_entity`, `entity_types: [proper_name, place, named_entity]`, structured `name_analysis`, media provenance separation, and per-language literary translation metadata. Its Chinese poem is the original, English and French are literary translations, and Tibetan is a translation draft pending native-speaker review; ordinary lexical records remain valid without these fields.

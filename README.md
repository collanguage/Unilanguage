...
Semantic Mapper, Dictionary and Search consume the same `search_terms` and structured entries. Dataset v1.2.9 contains 37 records. English and French browse forms are alphabetized; the Chinese browse index shows one concise preferred form per unified record in Xinhua-style stroke order. Full synonyms remain searchable and preserved inside each record. Sky displays `sky ↔ 盖 gài · 天空`; Universe displays `universe ↔ 斡 wò · 宇宙`; Sound displays `sound ↔ 声 shēng · 声音`; Abdomen displays `abdomen ↔ 肚 dù · 肚子`; Aback displays `aback ↔ 背 bèi · 吃惊地` and publishes “文明在你背后”; Abash now displays the low-confidence author candidate `abash ↔ 拍 pāi · 使窘迫`, distinguishes the independent word `bash` and the emotion candidate `怕 pà`, and publishes Jinkai Liu’s Collanguage prose “通向宇航时代的语言桥梁.” Featured forms and standard translations remain distinct, and historical boundaries are independently graded. The existing Universe record retains its Schema v1.0-compatible `root_level_mapping` extension. Legacy Website Import Batch 001 processes the first 20 source pages from the `languagesbook.com` English glossary into 14 new unified records, merges four sources into existing records, combines the two legacy `abbreviate/abbreviation` pages, and defers the unclear `cun，存` item to the Research Queue. No imported resemblance claim is automatically established. Original Jinkai Liu notes, independent historical evaluation, four evidence tracks, counterevidence and priority scores remain separate. Rebuild with `scripts/build-language-book-v1.mjs`, validate with `scripts/validate-language-book-v1.mjs`, and run the full test suite.

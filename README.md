...
Semantic Mapper, Dictionary and Search consume the same `search_terms` and structured entries. Dataset v1.2.0 contains 37 records. Legacy Website Import Batch 001 processes the first 20 source pages from the `languagesbook.com` English glossary into 14 new unified records, merges four sources into existing `AT`, `abbey`, `abdomen` and `aberrant` records, combines the two legacy `abbreviate/abbreviation` pages, and defers the unclear `cun，存` item to the Research Queue. No new record is automatically Published. Original Jinkai Liu notes, independent historical evaluation, four evidence tracks, counterevidence and priority scores remain separate. Rebuild the legacy batch with `scripts/build-legacy-website-import-batch-001.mjs`, then rebuild the dataset with `scripts/build-language-book-v1.mjs`, validate with `scripts/validate-language-book-v1.mjs`, and run the full test suite. See `docs/legacy-website-import-batch-001.md` for the reconciliation dashboard and historical corrections.

...
Semantic Mapper, Dictionary and Search consume the same `search_terms` and structured entries. Dataset Expansion v1 contains eleven records: nine migrated records (Sky, Universe, Human/Man, Sound, Language, Water, Advance, Light and AT · 在 · 爱) plus the reviewed `abbey → 修道院` and `aberrant → 反常的／偏离常规的` samples. The latter keeps `err ↔ 讹` as a low-confidence Candidate and rejects the consonant-bridge account only as historical morphology. New records are independently reviewable files in `data/entries/`; the build script merges them deterministically without discarding the v0.6 migration objects. Rebuild with `scripts/build-language-book-v1.mjs`, validate with `scripts/validate-language-book-v1.mjs`, and run `tests/language-book-v1.test.cjs`. See `docs/language-book-entry-schema-v1.md` for fields, enums, policy and the batch workflow.

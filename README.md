...
Semantic Mapper, Dictionary and Search consume the same `search_terms` and structured entries. Dataset Expansion v1.0.4 contains thirteen records: nine migrated records, the reviewed `abbey`, `aberrant` and `abdomen` samples, and the published `Namcha Barwa｜南迦巴瓦峰` Named Entity / Cultural-Literary Entry. The named-entity record includes Tibetan གནམ་ལྕགས་འབར་བ།, Wylie `gnam lcags ’bar ba`, source-linked morpheme analysis, graded Chinese translation assessments, a licensed terrain photograph and a separately labelled AI literary visualization. It uses optional Schema v1.0-compatible fields, keeps cultural interpretations source-attributed, and does not create a phonetic-semantic candidate. `err ↔ 讹` and `abdomen ↔ 肚 dù` remain low-confidence Candidates; rejected historical morphology and speculative associations stay on their own evidence tracks. New records are independently reviewable files in `data/entries/`; the build script merges them deterministically without discarding the v0.6 migration objects. Rebuild with `scripts/build-language-book-v1.mjs`, validate with `scripts/validate-language-book-v1.mjs`, and run `tests/language-book-v1.test.cjs`. See `docs/language-book-entry-schema-v1.md` for fields, enums, policy and the batch workflow.

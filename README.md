...
Semantic Mapper, Dictionary and Search consume the same `search_terms` and structured entries. Dataset Expansion v1.1.0 contains 23 records: the prior 13 records plus 10 independently reviewed Batch 001 records covering `generate`, `form`, `media`, `sign/cognition`, `montrer/monitor`, `fil/filière/fille`, `figure/finger`, `marchand/march`, `press/pression` and `convent/convention`. Six are editorially triaged as Reviewed and four retain Candidate mapping status; none is automatically Published. Original Jinkai Liu notes, independent historical evaluation, four evidence tracks, counterevidence and priority scores remain separate. `wave/water ↔ 水/w` is deferred to the Research Queue, while the existing `advance ↔ 往` calibration record is referenced without duplication. The earlier `abbey`, `aberrant`, `abdomen` and `Namcha Barwa｜南迦巴瓦峰` boundaries remain unchanged. New records are independently reviewable files in `data/entries/`; the build script merges them deterministically without discarding the v0.6 migration objects. Rebuild Batch 001 with `scripts/build-dataset-expansion-batch-001.mjs`, then rebuild the dataset with `scripts/build-language-book-v1.mjs`, validate with `scripts/validate-language-book-v1.mjs`, and run `tests/language-book-v1.test.cjs` plus `tests/dataset-expansion-batch-001.test.cjs`. See `docs/dataset-expansion-batch-001.md` for the dashboard and historical corrections.

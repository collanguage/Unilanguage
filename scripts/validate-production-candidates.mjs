...
const freeze = read('data/review/production-001-freeze.v0.1.json');
export function validateProduction(active, archive) {
 const errors=[];
 const check=(condition,message)=>{if(!condition)errors.push(message);};
 const expected=freeze.records;
 const ids=new Set();
 for (const [batch,id,count,archived] of [[active,'PRODUCTION-CANDIDATE-001',7,false],[archive,'PRODUCTION-ARCHIVE-001',1,true]]) {
  ...
  for(const r of batch?.records || []) {
   ...
   const contract=expected.find(x=>x.candidate_id===r.candidate_id);
   ...
   check((r.source_word==='gan')===archived,'archive/active contamination');
   ...
   check(r.source_provenance==='data/review/production-001-freeze.v0.1.json#'+r.candidate_id,'provenance missing');
...
if(process.argv[1]===fileURLToPath(import.meta.url)) {
 ...
 else console.log('Production import structure + Evidence/Editorial gate PASS: 7 Candidate, 1 Archive; 0 Reviewed/Published promotions.');
}

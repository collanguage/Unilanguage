...
for(const slug of ['abbreviate', 'abbreviation', 'abdomen', 'abdominal']) {
 ...
 const reader=`<section class="word-hero hero"><p>Language Book · Author: Jinkai Liu</p><h1>${esc(e.title.en)}</h1><p class="standard-translation"><strong>Standard Translation: ${esc(e.standard_translation?.target||e.primary_mapping.target.word)}</strong></p><p><strong>Featured Mapping: ${esc(f.target)} ${esc(f.reading)}</strong></p>${model.renderFeatured(e)}<p>Featured 是编辑选择，不是证据等级。Historical Relation: Not claimed。</p></section><section class="word-section section" id="stage-mappings"><h2>Diachronic Stage Mappings｜重要阶段</h2>${model.renderCards(e,'#research')}</section>`;

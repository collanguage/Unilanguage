...
Package G is stored in `package-g-decision-register.v0.1.json` and validated against `package-g-decision.schema.json`. It preserves Package F as an immutable intake layer, then adds evidence-completeness checks, a signed-decision contract, decision reasons and remediation, a separate publication eligibility checklist, explicit publisher approval fields, and append-only audit events. `review-decisions.html` is read-only. Package G v0.1 contains no signed decision or publication approval, so all 19 records remain Pending and Not Eligible.

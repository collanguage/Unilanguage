(function(root){
 ...
 const isPilot=e=>['second-pipeline-0.1','final-structural-0.1'].includes(e?.legacy_migration?.version);
 function render(e,prefix='') {
  ...
  const research=Object.fromEntries(['historical_etymology','evidence','hypotheses','experiments','name_analysis','cultural_associations','related_words','source',...(e.legacy_migration?.version==='final-structural-0.1'?['research_candidate_discovery','phonetic_observation','semantic_structure']:[]),'legacy_migration'].filter(k=>e[k]).map(k=>[k,e[k]]));

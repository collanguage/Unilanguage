(function () {
  ...
  function renderDictionary(entries) {
    ...
    grid.innerHTML = entries.map((entry) => { const mapping = entry.primary_mapping; return `<article class="card word-card" data-word="${escapeHtml(entry.search_terms.join(" "))}"><p class="label">${escapeHtml(entry.id)}</p><h2>${escapeHtml(mapping.source.word)} ↔ ${escapeHtml(mapping.target.word)}</h2><p>${escapeHtml(mapping.meaning.en)}</p><p lang="zh-Hans">${escapeHtml(mapping.meaning["zh-Hans"])}</p><p><span class="status">Entry ${escapeHtml(entry.entry_status)}</span> <span class="status cultural">Mapping ${escapeHtml(entry.mapping_status)} · Level ${escapeHtml(entry.mapping_level)}</span></p><p><strong>Historical relation · 历史关系：</strong> ${escapeHtml(entry.historical_relation_status)}<br><strong>Literary layer · 文学层：</strong> ${escapeHtml(entry.literary_layer.status)}</p>${entry.page ? `<a href="${escapeHtml(entry.page)}">Read entry · 阅读词条 →</a><br>` : ""}<a href="semantic-mapper.html?q=${encodeURIComponent(mapping.source.word)}">Open in Mapper · 在映射器中打开 →</a></article>`; }).join("");
  }

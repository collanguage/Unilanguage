(function () {
  ...
  function renderDictionary(entries) {
    ...
    grid.innerHTML = entries.map((entry) => { const separateTranslation = entry.slug === "abash" && entry.translation_status; const mapping = entry.primary_mapping, featured = entry.featured_mapping || entry.root_level_mapping?.featured_structural_mapping, isNamedEntity = entry.record_kind === "named_entity"; const heading = separateTranslation ? (featured ? `${escapeHtml(featured.source)} ↔ ${escapeHtml(featured.target)} <small>${escapeHtml(featured.reading)}</small>` : escapeHtml(mapping.source.word)) : featured ? `${escapeHtml(featured.source)} ↔ ${escapeHtml(featured.target)} <small>${escapeHtml(featured.reading)}</small> · ${escapeHtml(mapping.target.word)}` : `${escapeHtml(mapping.source.word)} ↔ ${escapeHtml(mapping.target.word)}`; return `<article class="card word-card" data-word="${escapeHtml(entry.search_terms.join(" "))}"><p class="label">${isNamedEntity ? "Named Entity / Literary Entry · 专名／文学词条" : escapeHtml(entry.id)}</p><h2>${heading}</h2>${separateTranslation ? `<p><strong>Standard Translation · ${escapeHtml(entry.translation_status)}：</strong>${escapeHtml(mapping.gloss["zh-Hans"])}</p><p><strong>Active Association${featured ? "：ABASH ↔ BASH" : " → BASH"}</strong> · ${escapeHtml(entry.active_association_status)} · Cognitive / Pedagogical · NOT etymology</p>` : featured ? `<p><strong>Standard translation · 通用翻译：</strong> ${escapeHtml(mapping.target.word)}</p>` : ""}<p>${escapeHtml(mapping.meaning.en)}</p><p lang="zh-Hans">${escapeHtml(mapping.meaning["zh-Hans"])}</p><p><span class="status">Entry ${escapeHtml(entry.entry_status)}</span> <span class="status cultural">${isNamedEntity ? "Named Entity · Place" : `Mapping ${separateTranslation ? `${escapeHtml(featured?.source || "bash")} ↔ ${escapeHtml(featured?.target || "拍")} · ` : ""}${escapeHtml(entry.mapping_status)} · Level ${escapeHtml(entry.mapping_level)}`}</span></p><p><strong>Historical relation · 历史关系：</strong> ${escapeHtml(entry.historical_relation_status)}<br><strong>Literary layer · 文学层：</strong> ${escapeHtml(entry.literary_layer.status)}</p>${entry.page ? `<a href="${escapeHtml(entry.page)}">Read entry · 阅读词条 →</a><br>` : ""}<a href="semantic-mapper.html?q=${encodeURIComponent(mapping.source.word)}">Open in Mapper · 在映射器中打开 →</a></article>`; }).join("");
  }

(function () {
  ...
  function renderEntry(entry) {
    ...
    const headerMapping = isAbandon
      ? `<h2>${escapeHtml(mapping.source.word)} ↔ <span lang="zh-Hans">${escapeHtml(featured.target)}</span> <small>${escapeHtml(featured.reading)}</small> <span class="headline-translation" lang="zh-Hans">· 放弃</span></h2><p class="featured-mapping-label">Featured structural and sound–meaning candidate · 特色结构与音义候选 <span>Candidate · Low confidence · Historical relation: ${escapeHtml(featured.historical_relation)}</span></p><p class="standard-translation"><span>Standard translation · 通用翻译</span><strong lang="zh-Hans">放弃</strong> <small>fàngqì</small></p><p><strong>Historical unit · 历史单位：${escapeHtml(featured.source)} ↔ ${escapeHtml(featured.target)} ${escapeHtml(featured.reading)}</strong><br>The headline presents the entry’s candidate; the comparison is with historical bandon.｜标题展示本词条的候选；实际比较对象为历史单位 bandon，办不是 abandon 的标准译词。</p><h3>Historical Semantic Path · 历史语义路径</h3><p>${escapeHtml(entry.diachronic_semantic_mapping.historical_path)}</p><p>${escapeHtml(entry.diachronic_semantic_mapping.semantic_path)}</p><p>Meaning first, consonant second.｜先定语义，再验辅音。 · <a href="${escapeHtml(entry.page)}#bandon-ban">Evidence and comparison · 查看证据与比较</a></p>`
      : separateTranslation

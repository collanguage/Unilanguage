(function () {
  ...
  function renderEntry(entry) {
    ...
    const separateTranslation = entry.slug === "abash" && entry.translation_status;
    const featured = entry.featured_mapping || entry.root_level_mapping?.featured_structural_mapping;
    const activeAssociations = (entry.related_words || []).filter((item) => item.relationship_type.startsWith("Active Association Principle /"));
    const activeAssociationSummary = activeAssociations.map((item) => `<p class="evidence-boundary"><strong>Active Association · 主动联系：${escapeHtml(item.word.toUpperCase())}</strong><br><span class="status-chip">${escapeHtml(item.status)}</span><br>Cognitive / Pedagogical · 认知与记忆桥梁 · NOT etymology / 非历史词源关系<br>${escapeHtml(item.word)} · <a href="${escapeHtml(entry.page)}#active-association">Read association · 查看主动联系</a></p>`).join("");
    ...
    const isAbandon = entry.slug === "abandon";
    // PILOT-HEADER:START
    const pilot = globalThis.UnilanguageDiachronic?.isPilot(entry);
    const headerMapping = pilot
      ? `<h2>${escapeHtml(UnilanguageDiachronic.headline(entry))}</h2><p class="standard-translation">Standard Translation: ${escapeHtml(standardGloss)}</p><p>Featured selection is editorial, not an evidence grade. · Historical Relation: Not claimed</p>${entry.legacy_migration?.version === "pending-final-0.1" && !featured ? "<p>Featured Mapping is optional. Pending 是有效研究结果，不是页面错误。</p>" : ""}${UnilanguageDiachronic.renderFeatured(entry)}${UnilanguageDiachronic.renderCards(entry, entry.page + "#research")}`
      : isAbandon // PILOT-HEADER:END
      ? `<h2>${escapeHtml(mapping.source.word)} ↔ <span lang="zh-Hans">${escapeHtml(featured.target)}</span> <small>${escapeHtml(featured.reading)}</small></h2><p class="featured-mapping-label">Whole-word mapping · 整词映射 · LET GO / RELEASE → 放 <span>Candidate · Low confidence · Historical relation: ${escapeHtml(featured.historical_relation)}</span></p><p class="standard-translation"><span>Standard translation · 通用翻译 · ${escapeHtml(entry.translation_status)}</span><strong lang="zh-Hans">${escapeHtml(mapping.target.word)}</strong> <small>${escapeHtml(mapping.target.pronunciation)}</small></p><p>Meaning first, consonant second.｜先定语义，再验辅音。<br>先依据 LET GO / RELEASE 选择放，再比较 abandon 中的 b ↔ 放 f：b-p-m-f Candidate group。Historical Relation: Not claimed。</p><h3>Historical unit / Root Mapping Candidate · 历史单位／词根映射候选</h3><p><strong>bandon ↔ 办 bàn</strong><br>Phonetic-Semantic Candidate · Low confidence · Historical Relation: Not claimed · <a href="${escapeHtml(entry.page)}#bandon-ban">Root evidence · 词根证据</a></p><h3>Historical Semantic Path · 历史语义路径</h3><p>${escapeHtml(entry.diachronic_semantic_mapping.historical_path)}</p><p>${escapeHtml(entry.diachronic_semantic_mapping.semantic_path)}</p><p><a href="${escapeHtml(entry.page)}#whole-word-mapping">Whole-word evidence · 整词证据</a></p>`
      : separateTranslation
      ? `${featured ? `<h2>${escapeHtml(featured.source)} ↔ ${escapeHtml(featured.target)} <small>${escapeHtml(featured.reading)}</small> <span class="headline-translation" lang="zh-Hans">· ${escapeHtml(standardGloss)}</span></h2><p>${escapeHtml(featured.display_label)} · ${escapeHtml(featured.status)}</p>` : `<h2>${escapeHtml(mapping.source.word)}</h2>`}<p class="standard-translation"><span>Standard Translation · 标准翻译 · ${escapeHtml(entry.translation_status)}</span><strong lang="zh-Hans">${escapeHtml(mapping.gloss["zh-Hans"])}</strong></p>${mapping.source.word === "abash" ? `<p><strong>Active Association → <a href="semantic-mapper.html?q=bash">BASH</a></strong></p>` : ""}`
      : featured
      ? `<h2>${escapeHtml(featured.source)} ↔ <span lang="zh-Hans">${escapeHtml(featured.target)}</span> <small>${escapeHtml(featured.reading)}</small> <span class="headline-translation" lang="zh-Hans">· ${escapeHtml(standardGloss)}</span></h2><p class="featured-mapping-label">${escapeHtml(featured.display_label || "Featured structural mapping · 特色结构映射")} <span>${escapeHtml(featured.status)} · Historical relation: ${escapeHtml(featured.historical_relation)}</span></p><p class="standard-translation"><span>Standard translation · 通用翻译</span><strong lang="zh-Hans">${escapeHtml(entry.standard_translation?.target || (entry.slug === "abdomen" ? standardTranslations : mapping.target.word))}</strong> ${(entry.standard_translation || entry.slug === "abdomen") ? "" : `<small>${escapeHtml(mapping.target.pronunciation)}</small>`}</p>`
      : `<h2>${escapeHtml(mapping.source.word)} ↔ ${escapeHtml(mapping.target.word)}${entry.slug === "new" ? ` <small>${escapeHtml(mapping.target.pronunciation)}</small> <span class="headline-translation" lang="zh-Hans">· ${escapeHtml(entry.languages.find(form => form.role === "standard-translation" && form.code === "zh-Hans").word)}</span>` : ""}</h2><p class="entry-language">${escapeHtml(mapping.source.language)} ${escapeHtml(mapping.source.pronunciation)} → ${escapeHtml(mapping.target.language)} ${escapeHtml(mapping.target.pronunciation)}</p>`;
    ui.state.innerHTML = ""; ui.result.hidden = false;
    ui.result.innerHTML = `<header class="entry-header"><div><p class="mapper-kicker">${recordLabel} · Schema v1.0</p>${headerMapping}${isAbandon ? "" : activeAssociationSummary}</div><span class="record-badge status-${entry.entry_status.toLowerCase()}">${isNamedEntity ? "Named Entity<br>Literary Entry" : `${escapeHtml(entry.entry_status)}<br>Entry status only`}</span></header>
      <div class="status-grid">${statusCard("Entry Status · 词条", entry.entry_status, entry.entry_status === "Published" ? "published" : "candidate")}${separateTranslation ? `${statusCard("Translation Status · 标准翻译", entry.translation_status)}${statusCard(`Mapping Status · ${featured?.source || "bash"} ↔ ${featured?.target || "拍"}`, entry.mapping_status)}${statusCard("Active Association Status · 主动联系", entry.active_association_status)}` : statusCard(featured ? "Translation Status · 翻译" : "Mapping Status · 映射", featured && entry.translation_status ? entry.translation_status : entry.mapping_status)}${statusCard(separateTranslation ? `Mapping Level · ${featured?.source || "bash"} ↔ ${featured?.target || "拍"}` : "Mapping Level · 层级", separateTranslation ? `${entry.mapping_level} / Low confidence` : entry.mapping_level)}${statusCard("Historical Relation · 历史关系", entry.historical_relation_status, "unestablished")}${statusCard("Literary Layer · 文学层", literature.status, literature.status === "Published" ? "published" : "planned")}</div>

(function () {
  ...
  function renderEntry(entry) {
    ...
    ui.result.innerHTML = `<header class="entry-header"><div><p class="mapper-kicker">${recordLabel} · Schema v1.0</p>${headerMapping}${isAbandon ? "" : activeAssociationSummary}</div><span class="record-badge status-${entry.entry_status.toLowerCase()}">${isNamedEntity ? "Named Entity<br>Literary Entry" : `${escapeHtml(entry.entry_status)}<br>Entry status only`}</span></header>
      <div class="status-grid">${statusCard("Entry Status · 词条", entry.entry_status, entry.entry_status === "Published" ? "published" : "candidate")}${separateTranslation ? `${statusCard("Translation Status · 标准翻译", entry.translation_status)}${statusCard(`Mapping Status · ${featured?.source || "bash"} ↔ ${featured?.target || "拍"}`, entry.mapping_status)}${statusCard("Active Association Status · 主动联系", entry.active_association_status)}` : statusCard(featured ? "Translation Status · 翻译" : "Mapping Status · 映射", entry.mapping_status)}${statusCard(separateTranslation ? `Mapping Level · ${featured?.source || "bash"} ↔ ${featured?.target || "拍"}` : "Mapping Level · 层级", separateTranslation ? `${entry.mapping_level} / Low confidence` : entry.mapping_level)}${statusCard("Historical Relation · 历史关系", entry.historical_relation_status, "unestablished")}${statusCard("Literary Layer · 文学层", literature.status, literature.status === "Published" ? "published" : "planned")}</div>

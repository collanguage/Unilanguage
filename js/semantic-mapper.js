(function () {
  ...
  function renderEntry(entry) {
    ...
    const headerMapping = isAbandon
      ? `<h2>ABANDON · <span lang="zh-Hans">放弃</span></h2><p class="standard-translation"><span>ABANDON · Standard Translation · 标准翻译 · ${escapeHtml(entry.translation_status)}</span><strong lang="zh-Hans">${escapeHtml(mapping.target.word)}</strong></p><h3>Historical Semantic Path · 历史语义路径</h3><p>${escapeHtml(entry.diachronic_semantic_mapping.historical_path)}</p><p>${escapeHtml(entry.diachronic_semantic_mapping.semantic_path)}</p><h3>Root Mapping Candidate · 词根映射候选</h3><p class="formula"><strong>${escapeHtml(featured.source)} ↔ ${escapeHtml(featured.target)} ${escapeHtml(featured.reading)}</strong></p><p>${escapeHtml(featured.status)} · Historical Relation: ${escapeHtml(featured.historical_relation)}</p><p>Meaning first, consonant second.｜先定语义，再验辅音。 · <a href="${escapeHtml(entry.page)}#bandon-ban">Evidence and comparison · 查看证据与比较</a></p>`
      : separateTranslation

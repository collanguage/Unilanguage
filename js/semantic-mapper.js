(function () {
  ...
  function renderEntry(entry) {
    ...
    const headerMapping = separateTranslation
      ? `${featured ? `<h2>${escapeHtml(featured.source)} ↔ ${escapeHtml(featured.target)} <small>${escapeHtml(featured.reading)}</small></h2><p>${escapeHtml(featured.display_label)} · ${escapeHtml(featured.status)}</p>` : `<h2>${escapeHtml(mapping.source.word)}</h2>`}<p class="standard-translation"><span>Standard Translation · 标准翻译 · ${escapeHtml(entry.translation_status)}</span><strong lang="zh-Hans">${escapeHtml(mapping.gloss["zh-Hans"])}</strong></p>${!featured ? `<p><strong>Active Association → <a href="semantic-mapper.html?q=bash">BASH</a></strong></p>` : ""}`
      : featured
    ...
    ui.result.innerHTML = `<header class="entry-header"><div><p class="mapper-kicker">${recordLabel} · Schema v1.0</p>${headerMapping}${activeAssociationSummary}</div><span class="record-badge status-${entry.entry_status.toLowerCase()}">${isNamedEntity ? "Named Entity<br>Literary Entry" : `${escapeHtml(entry.entry_status)}<br>Entry status only`}</span></header>
      <div class="status-grid">${statusCard("Entry Status · 词条", entry.entry_status, entry.entry_status === "Published" ? "published" : "candidate")}${separateTranslation ? `${statusCard("Translation Status · 标准翻译", entry.translation_status)}${statusCard("Mapping Status · bash ↔ 拍", entry.mapping_status)}${statusCard("Active Association Status · 主动联系", entry.active_association_status)}` : statusCard(featured ? "Translation Status · 翻译" : "Mapping Status · 映射", entry.mapping_status)}${statusCard(separateTranslation ? "Mapping Level · bash ↔ 拍" : "Mapping Level · 层级", separateTranslation ? `${entry.mapping_level} / Low confidence` : entry.mapping_level)}${statusCard("Historical Relation · 历史关系", entry.historical_relation_status, "unestablished")}${statusCard("Literary Layer · 文学层", literature.status, literature.status === "Published" ? "published" : "planned")}</div>
